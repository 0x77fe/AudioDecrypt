...
//搜索文件
vector<filesystem::path> SerchFiles(const filesystem::path& Dir, const string& Suffix)
{
	auto files = vector<filesystem::path>();
	for (const auto& file : filesystem::recursive_directory_iterator(Dir))
	{
		if (filesystem::is_regular_file(file) and file.path().extension().string() == Suffix)
		{
...
#endif // WIN32
string win32(const string& utf8_str)
{
#ifdef WIN32
